...
public class HabitosService {
    ...
    public List<HabitosDTOResponse> buscaHabitosPorEmail(String token) {
        String email = jwtUtil.extrairEmailToken(token.substring(7));
        List<HabitosDTOResponse> lista = habitosConverter.paraListaDTOResponse(habitosRepository.findByEmail(email));
        if (lista.isEmpty()) {
            throw new ResourceNotFoundException("Nenhum habito com esse email");
        }
        return lista;
    }
}
